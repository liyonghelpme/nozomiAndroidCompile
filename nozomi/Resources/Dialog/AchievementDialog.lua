...
local function updateAchievementCell(bg, scrollView, info)
    ...
    if info.level<=3 then
        ...
        if num>=info.max then
            temp = UI.createButton(CCSizeMake(146, 43), onGetRewards, {callbackParam=info, image="images/buttonGreen.png", text=StringManager.getString("buttonReward"), fontSize=15, fontName="fonts/font3.fnt"})
        ...
        temp = UI.createLabel(StringManager.getString("labelAchieveComplete"), "fonts/font2.fnt", 15, {colorR = 157, colorG = 226, colorB = 76})
    ...
    temp = UI.createSpriteWithFile("images/achieveStar.png",CCSizeMake(59, 57))
    screen.autoSuitable(temp, {x=67, y=46})
    bg:addChild(temp)
    temp = UI.createSpriteWithFile("images/achieveStar.png",CCSizeMake(43, 40))
    screen.autoSuitable(temp, {x=37, y=41})
    bg:addChild(temp)
    temp = UI.createSpriteWithFile("images/achieveStar.png",CCSizeMake(43, 41))
...
function AchievementDialog.show()
    ...
    if General.useGameCenter then
        ...
        bg:addChild(temp)
        temp = UI.createLabel(StringManager.getString("labelGameCenterAchieve"), "fonts/font1.fnt", 13, {colorR = 0, colorG = 0, colorB = 0})
