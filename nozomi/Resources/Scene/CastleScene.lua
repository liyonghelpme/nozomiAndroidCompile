...
function CastleScene:initBuilds(initInfo)
    ...
    for i=1, bnum do
        ...
        local setting = {id=build.buildIndex, buildIndex=bcache[build.bid], initGridX=math.floor(build.grid/10000), initGridY=build.grid%10000, level=build.level, time=build.time}
...
function OperationScene:ctor(isVisit)
    if isVisit then
...
function OperationScene:initData(initInfo)
    ...
    while obsNum<40 and UserData.obstacleTime<t1 do
        obsNum = obsNum+1
    ...
    local zombieView = MapGridView.new(GRIDSIZEX, GRIDSIZEY, 930, 2212)
    ...
    zombieView:setGridUse(GridKeys.Build, 10, 4, 4, 1)
...
function OperationScene:updateLogic(diff)
    ...
    if not self.test then
        self.test = true
        EventManager.sendMessage("EVENT_NOTICE_BUTTON", {name="mail", isShow=true})
    end
    SoldierLogic.updateSoldierList()
    self.synTime = self.synTime+diff
    if not UserData.noPerson and self.synTime>30 then
...
function BattleScene:initData(initInfo)
    ...
    local areaAlpha = getParam("buildingAreaAlpha", 38)
    local lineAlpha = getParam("buildingLineAlpha", 100)
    self.mapGridView.blockBatch:runAction(CCAlphaTo:create(0.1, areaAlpha, areaAlpha))
    self.mapGridView.linesBatch:runAction(CCAlphaTo:create(0.1, lineAlpha, lineAlpha))
end
...
function ReplayScene:ctor(menuParam)
    ...
    math.randomseed(tonumber(self.replayFile:read()))
    local cmdList = {}
    while true do
        local cmd = json.decode(self.replayFile:read())
        table.insert(cmdList, cmd)
        if cmd[2]=="e" then
            self.battleTime = cmd[1]
            break
        end
    end
...
function ReplayScene:updateLogic(diff)
    if not self.pause then
        ...
        while not self.cmdTime or self.time>self.cmdTime do
            ...
            self.cmd = table.remove(self.cmdList, 1)
